...
// (ModbusRegisters 0 - 3 read only and ModbusRegisters 4 - 7 write only from the Master perspective):
// ModbusRegisters[0] - Read an analog value from the CONTROLLINO_A0 - returns value in the range from 0 to 1023.
// ModbusRegisters[1] - Read an digital value from the CONTROLLINO_D0 - returns only the value 0 or 1.
// ModbusRegisters[2] - Read the number of incoming messages - Communication diagnostic.
// ModbusRegisters[3] - Read the number of number of outcoming messages - Communication diagnostic.
// ModbusRegisters[4] - Sets the Relay output CONTROLLINO_R0 - only the value 0 or 1 is accepted.
// ModbusRegisters[5] - Sets the Relay output CONTROLLINO_R1 - only the value 0 or 1 is accepted.
// ModbusRegisters[6] - Sets the Relay output CONTROLLINO_R2 - only the value 0 or 1 is accepted.
// ModbusRegisters[7] - Sets the Relay output CONTROLLINO_R3 - only the value 0 or 1 is accepted.
...
void loop()
{
    ...
    // If the received frame is Ok the instance poll writes or reads corresponding values to the internal registers (ModbusRegisters).
    // Main parameters of the instance poll are address of the internal registers and number of internal registers.
    ControllinoModbusSlave.poll(ModbusRegisters, 8);
    ...
    ModbusRegisters[0] = analogRead(CONTROLLINO_A0);           // Read the analog input CONTROLLINO_A0.
    ModbusRegisters[1] = digitalRead(CONTROLLINO_D0);          // Read the digital input CONTROLLINO_A0.
    ModbusRegisters[2] = ControllinoModbusSlave.getInCnt();    // Read the number of incoming messages.
    ModbusRegisters[3] = ControllinoModbusSlave.getOutCnt();   // Read the number of outcoming messages

    digitalWrite(CONTROLLINO_R0, ModbusRegisters[4]); // Update the relay output CONTROLLINO_R0 - ON/OFF
    digitalWrite(CONTROLLINO_R1, ModbusRegisters[5]); // Update the relay output CONTROLLINO_R1 - ON/OFF
    digitalWrite(CONTROLLINO_R2, ModbusRegisters[6]); // Update the relay output CONTROLLINO_R2 - ON/OFF
    digitalWrite(CONTROLLINO_R3, ModbusRegisters[7]); // Update the relay output CONTROLLINO_R3 - ON/OFF
}
